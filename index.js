...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511941834382:+55 (11) 941834382\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo paulo! ❌',
    			...
    			const Whatis = ['Sim','Não']
        		const VocêPode = ['Pode','Não pode']
		        const QuandoÉo = ['Outro dia','Outro domingo','Outro mês','Outro ano']
			const botNumber = client.user.jid
			const ownerNumber = ["5511941834382@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5511941834382@s.whatsapp.net'
                        /******ApiKey Input******/
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'kbbi':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
              ...
              case 'VocêPode':
                client.updatePresence(from, Presence.composing) 
              if (!isUser) return reply(mess.only.daftarB)
                random = VocêPode[Math.floor(Math.random() * (VocêPode.length))]
	    ...
	    case 'QuandoÉo':
               client.updatePresence(from, Presence.composing) 
                if (!isUser) return reply(mess.only.daftarB)
               random = QuandoÉo[Math.floor(Math.random() * (QuandoÉo.length))]
                ...
                break
                   ...
                   break
					...
					anu = await fetchJson(`http://rt-files.000webhostapp.com/tts.php?apikey=rasitech`, {method: 'get'})
                                        if (!isUser) return reply(mess.only.daftarB)
					setTimeout( () => {
					client.sendMessage(from, '*➸ Resposta :* '+anu.result.Resposta+'\n'+anu.result.desk, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
